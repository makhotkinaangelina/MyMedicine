...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_category'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE categories SET Category = ? WHERE CategoryID = ?");
...
<div class="container">
    <form method="POST" class="doctor-form" action="">
        ...
        <div class="form-floating">
            ...
            <input type="text" id="name" class="form-control" name="name" required maxlength="70" value="<?php echo isset($_POST['name']) ? $_POST['name'] : ''; ?>"><br>
        </div>
        <div class="form-floating">
            <label for="email">Email врача:</label>
            <input type="email" id="email" class="form-control" name="email" required value="<?php echo isset($_POST['email']) ? $_POST['email'] : ''; ?>"><br>
        </div>
        <div class="form-floating">
            <label for="password">Пароль для входа в учетную запись:</label>
            <input type="password" id="password" class="form-control" name="password" required value="<?php echo isset($_POST['password']) ? $_POST['password'] : ''; ?>"><br>
        </div>
        <div class="form-floating">
        ...
        <div class="form-floating">
            ...
            <input type="date" id="date_birth" name="date_birth" required value="<?php echo isset($_POST['date_birth']) ? $_POST['date_birth'] : ''; ?>"><br>
        </div>    
        <div class="form-floating">    
            <label for="experience_start">Дата начала работы:</label>
            <input type="date" id="experience_start" name="experience_start" required value="<?php echo isset($_POST['experience_start']) ? $_POST['experience_start'] : ''; ?>"><br>
        </div>   
...
<div class="container">
    ...
    <form method="POST">
        ...
        <button class="btn btn-primary w-100 py-2 add-btn" type="submit" name="add_category">Добавить категорию</button>
    </form>
</div>
...
<div class="container">
    ...
    <form method="POST">
        ...
        <button class="btn btn-primary w-100 py-2 add-btn" type="submit" name="edit_category">Редактировать категорию</button>
    </form>
</div>
...
<div class="container">
    ...
    <label for="delete_category_id">Выберите категорию для редактирования:</label>
        ...
        <button class="btn btn-primary w-100 py-2 add-btn" type="submit" name="delete_category">Удалить категорию</button>
    </form>
</div>
